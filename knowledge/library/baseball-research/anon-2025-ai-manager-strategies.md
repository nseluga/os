...
- **Matchup model**: Marcel projections of per-PA outcome rates by handedness
  ...
  24×25 baserunner-out transition matrices (deterministic for HR/3B/BB/HBP/K,
...
- The infinite state space (any number of runs, endless extras) is truncated
  ...
  lineups are only known hours before first pitch.
- **Validation-by-history is the paper's best trick**: in 2,296 actual 2023
  MLB games, the manager whose decisions deviated less (in cumulative win
  probability cost) from the Lindsey strategy won 52.7% of the time
  (p = 0.0045). That establishes the baseline as real, so any AI that beats
  Lindsey in simulation inherits credibility.
- The half-game decomposition is justified *by the matchup model's own
  ...
  don't interact, so the game factorizes exactly.
- The pseudo-full game exists because runs ≠ wins: Lindsey's classic example
  ...
  expected runs allowed but *lowers* win probability. The chance node for the
  other half-inning's runs restores the win objective cheaply.
- Roster constraints (26-man roster, 6 of 9 starters locked in, 8 available
  relievers) mirror real MLB rules and pinch-hitting practice, and are what
  make equilibrium solving tractable in hours on one CPU core.
- Known gaps stated plainly: no pitcher fatigue (a 100-pitch starter looks
  identical to a fresh one) and no fielder skill — both flagged as the reason
  defensive-substitution nodes were excluded from evaluation.
...
- The **pseudo-full game manager is the champion**: 57.1% vs. Lindsey over
  ...
  baseline (48.3%) — naive tree search is not good enough for in-game use.
- Managers who managed closer to a 1963 operations-research strategy won a
  statistically significant majority of 2023 games — six decades later,
  Lindsey still beats gut feel.
- **Cash was wrong to pull Snell**: staying with Snell vs. Betts = 0.584 win
  ...
  reliever choice available, and fatigue modeling might narrow the gap.)
- **Boone was basically right**: his whole 10th inning cost the Yankees only
  ~0.0096 win probability. Cortes over Hill to face Ohtani was the *optimal*
  call (0.669 vs 0.627) despite the media criticism; the IBB to Betts cost
  0.009. "We encourage Yankees fans to be more forgiving."
