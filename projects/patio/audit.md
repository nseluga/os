...
**State of completeness** (updated 2026-07-05): React Native/Expo conversion complete — Stage M.1–M.3 done on `conversion` branch. The `mobile/` directory is a working Expo SDK 54 app with auth (SecureStore), all 5 screens ported, and API pointed at the Render backend. M.4 (web app retirement) is deferred until mobile is fully verified end-to-end. Backend stages 0–9 are next (dev-team-auto). Known 500 errors on `/pvp_bets`, `/cpu_bets`, `/ongoing_bets`, `/me` are pre-existing backend bugs in the plan (stages 0–5), not mobile regressions. `server.js` is still present (deletion deferred with M.4).
