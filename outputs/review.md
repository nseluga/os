...
<!-- Check the box next to each item you approve. Run /improve-system again to apply checked items. -->
  ...
  **Why:** no org-level secret sharing on personal GitHub accounts — each repo needs its own copy.
  ...
  ## Git workflow (observed 2026-07-27, l2detailz)

  For `bcns-client-l2detailz`, small changes go straight to `main` — Nate explicitly skipped `branch`/`ship` in three separate sessions the same day. Treat this as the working convention for this client repo (not necessarily every bcns repo) until told otherwise: don't default to cutting a branch for small fixes here.
  ```
