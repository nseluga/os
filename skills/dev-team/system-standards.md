...
## API Design
- **RESTful Conventions** — resources are nouns, verbs express action.
- **Consistent Error Contract** — all endpoints return errors in one shape (status + error field).
- **Backward Compatibility** — removing/renaming a response field is breaking → version it.
- **Versioning Strategy** — version from the start (`/v1/`).
- **Pagination on Collections** — list endpoints must paginate; returning all records is never the default.
- **Idempotency Keys** — on mutating endpoints over unreliable networks.
- **Status Codes are Semantic** — no 200 + `{"error"}`.
...
- **No Nullable FKs Without a Reason**
...
- **Cohesion Over Convenience** — group by domain concept.
...
- **Caching** — cache repeated expensive reads; start in-process (dict/LRU), reach for Redis only when cache must be shared across multiple processes or servers; measure before adding.
...
- **CDN / Static Edge** — serve all static assets (JS, CSS, images) through a CDN from day one; only move dynamic logic to the edge (Cloudflare Workers, etc.) when origin latency is a measured problem.
...
- **Load Balancing** — put a load balancer in front of multiple app instances before you need it.
- **Connection Pooling** — never open a new DB connection per request; use a pool (PgBouncer, built-in ORM pooling); pool exhaustion is a scaling cliff that appears suddenly at moderate traffic.
...
- **Vertical Before Horizontal** — exhaust vertical scaling (bigger instance, more RAM, faster disk) before adding distribution complexity.
