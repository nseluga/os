...
- **QA runs targeted tests, not the full suite, on every attempt.** First-attempt QA scopes to the tests it wrote plus existing tests covering the item's touched files (from the engineer report) — never the project's full test command. Re-gating after a fix narrows further: only the previously-failing checks plus tests covering the files in the fix report's Changes Made; repeat the live smoke pass only if the fix touched routes/models/migrations/serialization. State the scope in QA's spawn prompt; widen it at your discretion if a fix looks like it could ripple. Cross-item regressions are caught once, at `/dev-team-auto` Shut Down — not by re-running a growing suite on every item.
