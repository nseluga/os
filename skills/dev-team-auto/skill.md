...
## Outer Loop: For Each LANE.md Item

Starting from your resume point, for each item in execution order:

### 1. Check for a stop marker

If the current item sits at or past a LANE.md line beginning with `> **⚠️ AUTONOMOUS RUN — STOP HERE`, skip to **Shut Down** immediately.
...
- `LANE_PROGRESS.md` — append a dated entry for the item per the progress-md schema: `done [team] — [summary + commit hash]` or `blocked — [reason]` (never silently mark a blocked item done). One entry per item, written as soon as it finishes.
- `LANE.md` — set that item's `status:`.
...
3. Write a final LANE_PROGRESS.md entry for the run: items done, items blocked, next item.
