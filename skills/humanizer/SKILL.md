...
name: humanizer
description: |
  Rewrite AI-sounding text to remove LLM writing tells and match Nate's own
  voice via knowledge/library/style_reference/. Use when the user says
  "humanize this", "make this sound less like AI", "sound more like me", or
  asks you to rewrite/edit copy, a writeup, or site content before it ships.
...
Default sample source for Nate: `~/os/knowledge/library/style_reference/`. Read its `README.md` first — it distinguishes two kinds of reference:

- **Structural/rhythm reference** (`Project_Writeup.pdf`, `Seluga Final Paper.pdf`, and their `.md` notes files) — sentence variety, transitions, how he argues and builds to a point. Copy the habits, not the formality level.
- **Voice calibration examples** (`seluga-*.md` personal writeups) — register, how he explains decisions in his own words, what he emphasizes and skips.

Load both kinds before rewriting anything for Nate. If the content type has no matching example (e.g. a Slack message when only project-writeup samples exist), say so and ask for a sample rather than guessing register.

If the user provides a writing sample directly in the conversation instead (or this is for someone other than Nate), use that:

1. Read the sample(s) first. Note sentence lengths, vocabulary, paragraph openings, punctuation, recurring phrases, and transitions.
2. Match those habits instead of merely deleting AI patterns. Do not upgrade casual words or regularize deliberate quirks.
3. Without any sample, use the default behavior below.
